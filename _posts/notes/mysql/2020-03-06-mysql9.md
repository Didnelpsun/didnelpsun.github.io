...
我们也可以同时使用`CONCAT_WS()`或者`CONTACT()`，这两个函数在[数据类型：字符串的1.4CONCAT()与CONCAT_WS()]({% post_url notes/mysql/2020-03-05-mysql4 %})的字符串函数中。
...
等同于[CASE语句]({% post_url notes/mysql/2020-03-11-mysql15 %})的：
...
在[SQL教程的自动生成主键]({% post_url notes/sql/2019-07-08-sql3 %}中已经提过了AUTO_INCREMENT序列。
