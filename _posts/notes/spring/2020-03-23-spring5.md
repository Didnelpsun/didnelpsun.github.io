...
这也是之前我们最经常使用的setter方法。当容器调用一个无参的构造函数或一个无参的静态factory方法来初始化你的bean后，通过容器在你的bean上调用设值函数，基于设值函数的DI就完成了。具体的还可以看看[Java的Setter和Getter方法]({% post_url notes/java/2020-03-25-settergetter %})。
