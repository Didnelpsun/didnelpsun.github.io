...
同时也可以直接利用[生命周期]({% post_url notes/spring/2020-03-20-spring-life-cycle %})的property注解方式的`@PropertySource(properties配置文件路径字符串)`和`@Value(SpEL表达式)`来替代读取properties配置文件。当出现多个数据源就在方法的参数中使用`@Qualifier(数据源名)`来指定数据源。
