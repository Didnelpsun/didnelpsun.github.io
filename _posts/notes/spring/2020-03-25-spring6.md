...
我们这里会讲到[三层架构]({% post_url notes/spring/2020-01-29-introduction-and-build %})，我们暂时只会用到前面三种注释，对于三层结构的注释会在后面使用。

## @Component、@ComponentScan和@ComponentScans

### &emsp;@Component

我们之前在[基本容器与实例注入的注释]({% post_url notes/spring/2020-03-19-container-and-instance-injection %})已经用过了@Component和@ComponentScan来配置，这里我们并不会着重谈到。
