...
+ `hgetall 键名`：获取键中所有键域与键值。
+ `hdel 键名 键域1 键域2 ...`：删除一个或多个键域。
...
+ `zrem 键名 键值1 键值2 ...`：删除键中指定键值的元素。
+ `zremrangebylex 键名 最小键值 最大键值`：删除键中给定的字典区间的所有成员。按ASCII码对键值排序。
+ `zremrangebyrank 键名 开始索引 结束索引`：删除键中给定排名区间的所有成员。
+ `zremrangebyscore 键名 最小分数 最大分数`：删除键中给定分数区间的所有成员。
+ `zcount 键名 评分最小值 评分最大值`：统计键下分数区间内的元素个数。
+ `zrank 键名 键值`：返回该键值在键中的排名，从0开始。
+ `zscore 键名 键值`：返回该成员的评分。
...
Redis能通过Bitmaps类型对位进行操作，从而提高内存使用率和开发效率。Bitmaps本身不是一种数据类型，其实就是个字符串，但是可以对字符串的位进行操作。Bitmaps可以看作以位为单位的数组，数组的每个单位只能存放0和1，数组的下标称为偏移量，从0开始。

第一次初始化位图时，如果偏移量很大，那么初始化过程会很慢，从而造成Redis的堵塞，这时往往会将偏移量进行处理，如全部减去一个固定的值。

### &emsp;位图基本操作

+ `setbit 键名 偏移量 键值`：设置键中某个偏移量的值，为0或1。
+ `getbit 键名 偏移量`：获取键中某个偏移量的值，为0或1。
+ `bitcount 键名 开始偏移量 结束偏移量`：统计键中从开始偏移量到结束偏移量之间的字节比特值为1的数量。前后都包含。偏移量可以为负数值。
+ `bitop and/or/not/xor 目标键名 键名1 键名2 ...`：是一个符操作，可以对多个键名的位图进行交集、并集、非、异或操作并将结果保存到目标键名中。
...
Redis在2.8.9版本添加了HyperLogLog结构。为了解决基数问题，即求集合中不重复元素的个数。返回的是近似基数，对于大基数返回的不是准确值。

解决基数问题的方案：

1. 存储在SQL中，使用distinct count计算不重复个数。
2. 使用NoSQL的hast、set、bitmaps数据结构处理。

可以使用12KB内存就能计算2^64个不同元素的基数。但是由于只能根据输入元素来计算基数，而不会存储输入元素本身，所以不能像集合一样返回输入的各个元素。

### &emsp;基数统计基本操作

+ `pfadd 键名 键值1 键值2 ...`：添加键值到键中。返回值为添加后基数的变化，如果是新数据就返回新增的数值，如果有重复则返回不重复的增加值。
+ `pfcount 键名1 键名2 ...`：计算键的近似基数，可以计算多个键。
+ `pfmerge 目标键名 键名1 键名2 ...`：将一个或多个键合并后基数的结果存储到目标键中。
...
Redis在3.2中增加了对GEO类型的支持，即是元素的二维坐标，地图上就是经纬度。Redis提供了对经纬度的范围查询，距离查询，经纬度哈希表等操作的支持。

两极无法直接添加，一般下载城市数据，通过Java程序一次性导入。

有效的经度从-180到180，有效的维度从-85.05112878到85.05112878，当超出范围时会报错。

键域往往表示城市名称，不能有重复信息。

### &emsp;地理信息基本操作

+ `geoadd 键名 经度 维度 键域 ...`：添加一个或多个键域的经纬度信息。
+ `geopos 键名 键域1 键域2 ...`：获得指定键域的坐标值。
+ `geodist 键名 键域1 键域2 显示单位`：获得两个位置之间的直线距离。默认以米为单位，单位有：
  + m：米。
  + km：千米。
  + mi：英里。
  + ft：英尺。
+ `georadius 键名 经度 维度 半径 单位`：以给定的经纬度为中心，找到某一个半径内的所有值域。
...
Redis可以通过发布和订阅来实现消息队列，但是息无法持久化，如果出现网络断开、Redis 宕机等，消息就会被丢弃。
...
+ `xadd 键名 消息ID 键域1 键值1 键域2 键值2 ...`：向队列添加消息，如果指定的队列不存在，则创建一个队列。消息ID可以使用*表示由Redis生成，可以自定义，但是必须大于0且自己保证递增性。返回值为消息ID。
+ `xtrim 键名 maxlen 长度值`：修剪键值，让键值中只有最大长度值个消息。
+ `xdel 键名 消息ID1 消息ID2 ...`：删除键名中对应的消息。返回值为删除数量。
+ `xlen 键名`：获取键中包含的键值数量，即消息总长度。
+ `xrange 键名 消息ID开始值 消息ID结束值`：获取消息ID开始值和消息ID结束值之间的键值。消息ID开始值为-表示最小值，消息ID结束值为+表示最大值。
+ `xrange 键名 - + count 数量`：获取所有消息中从小到大的对应数量的消息值。
+ `xrevrange 键名 消息ID结束值 消息ID开始值`：与xrange相反，获取倒序的值，结束值为+表示最大值，结束值-为最小值。
+ `xrevrange 键名 + - count 数量`：获取所有消息中从大到小的对应数量的消息值。
+ `xread count 数量 streams 键名1 键名2 ... 消息ID1 消息ID2 ...`：以非阻塞方式获取对应数量的消息列表。
+ `xread count 数量 block 毫秒数 streams 键名1 键名2 ... 消息ID1 消息ID2 ...`：以阻塞方式获取对应数量的消息列表。
...
ReJSON是由Redis Labs开发的，[源码下载地址](https://github.com/RedisLabsModules/ReJSON)。

### &emsp;安装

以Ubuntu为例：

1. 安装系统环境：`apt-get install build-essential`。
2. 安装redisjson插件：`git clone https://github.com/RedisJSON/RedisJSON.git`。
3. 执行`make`编译，假设redisjson的路径为：/opt/redisjson。
4. 让Redis加载redisjson插件：`redis-server --loadmodule /opt/redisjson/src/rejson.so`。
